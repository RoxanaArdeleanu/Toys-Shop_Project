document.addEventListener('DOMContentLoaded', () => {
  ...
  //Function to get building toys from db

  const getBuildingBlocks = function () {
    fetch('http://localhost:3000/toys')
      .then(response => response.json())
      .then(data => {
        const plushToys = data.filter(toy => toy.categoryId === 4);
        console.log('Filtered building blocks:', plushToys);
