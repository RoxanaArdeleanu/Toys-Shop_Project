...
addFeaturedToys();
  ...
  //Function to get board-games from db
  const getBoardGames = function () {
    fetch('http://localhost:3000/toys')
      .then(response => response.json())
      .then(data => {
        const plushToys = data.filter(toy => toy.categoryId === 6);
        console.log('Filtered board games:', plushToys);
