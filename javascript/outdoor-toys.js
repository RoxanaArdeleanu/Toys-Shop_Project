...
//Function to get outdoor-toys from db
  const getOutdoorToys = function () {
    fetch('http://localhost:3000/toys')
      .then(response => response.json())
      .then(data => {
        const plushToys = data.filter(toy => toy.categoryId === 7);
        console.log('Filtered outdoor toys:', plushToys);
